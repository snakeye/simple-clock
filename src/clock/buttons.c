...
uint16_t button_status[BUTTONS_NUMBER] = {0};
uint16_t button_press_time[BUTTONS_NUMBER] = {0};
uint8_t button_on_hold[BUTTONS_NUMBER] = {0};
...
void buttons_tick(uint16_t tick)
{
	...
	//uint8_t button[2];
	for(uint8_t i = 0; i < BUTTONS_NUMBER; i++) {
		button_status[i] = (button_status[i]<<1) | !button[i] | 0xf000;
